...
基于正原电子ESP32S3开发板的智能天气时钟项目，采用ESP-IDF v5.3.1框架开发，集成WiFi连接、NTP时间同步、实时天气数据获取和TFT显示功能。
...
#### 天气API配置
编辑 `main/weather_client.h`：
```c
#define API_HOST            "devapi.qweather.com"
#define API_KEY             "your_qweather_api_key"  // 从和风天气官网获取
#define LOCATION_ID         "your_city_id"           // 城市ID（如北京：101010100）
```
...
void led_set_state(led_state_t state);  // 设置LED状态
led_state_t led_get_state(void);        // 获取LED状态
void led_toggle(void);                  // 翻转LED状态
```

**特性：**
- 低电平有效（LED_ON=0，LED_OFF=1）
- 500ms定时器闪烁
- 线程安全的状态管理
...
# 天气数据获取日志
I (6567) WEATHER_CLIENT: Fetching weather data from: https://devapi.qweather.com/v7/weather/now?location=...
